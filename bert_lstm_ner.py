...
flags.DEFINE_string(
    "bert_config_file", os.path.join(bert_path, 'bert_config_predict.json'),
...
flags.DEFINE_string(
    "output_dir", os.path.join(root_path, 'bert/output'),
...
flags.DEFINE_float('droupout_rate', 1.0, 'Dropout rate')
...
def convert_single_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    ...
    return feature
def convert_single_predict_example(ex_index, example, label_list, max_seq_length, tokenizer, mode):
    """
    将一个样本进行分析，然后将字转化为id, 标签转化为id,然后结构化到InputFeatures对象中
    :param ex_index: index
    :param example: 一个样本
